...
def auto_create_daily_goal(db: Session, monthly_goal: Goal):
    ...
    existing_daily = db.query(Goal).filter(Goal.timeframe == TimeframeType.TODAY).first()
    if existing_daily:
        existing_daily.target_profit = daily_goal_amount
    else:
        existing_daily = Goal(timeframe=TimeframeType.TODAY, target_profit=daily_goal_amount)
...
def create_goal(goal: GoalCreate, db: Session = Depends(get_db)):
    ...
    if existing.timeframe == TimeframeType.THIS_MONTH:
        auto_create_daily_goal(db, existing)
...
def update_goal(timeframe: str, goal: GoalUpdate, db: Session = Depends(get_db)):
    ...
    if db_goal.timeframe == TimeframeType.THIS_MONTH:
        auto_create_daily_goal(db, db_goal)
