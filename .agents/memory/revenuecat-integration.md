...
- **The custom `FallbackPaywall` is a React Native `<Modal>`, so it CANNOT be presented from inside another RN `<Modal>` on iOS.** iOS only lets one modal present per view controller at a time; calling `presentPaywall()`/`openFallback()` (which toggles the root-level FallbackPaywall Modal) while the Settings sheet or Analytics modal is open makes it **silently do nothing** (no throw, no UI). The native `RevenueCatUI.presentPaywall()` did NOT have this problem because it presents over everything natively.
  **Why:** the in-app "Upgrade to Pro" button lives inside the Settings pageSheet Modal; after switching to the custom paywall it became a dead button.
  **How to apply:** to open the custom paywall from inside a modal, close that modal first and call `presentPaywall()` in the modal's `onDismiss` (iOS-only callback, fires after dismissal fully completes — avoids the present-while-dismissing race). Don't use a fixed `setTimeout`. Same caveat applies to any gated action (CSV export, analytics) triggered from within a modal.
