...
- **RevenueCat dashboard config is PROVISIONED ✅ (Jun 26).** `scripts/seedRevenueCat.ts` ran successfully via the now-healthy connector. Project `Earnings Ninja` (`proj08f4330c`) has 3 apps (test/app store/play store), 9 products (lifetime *non_consumable* / yearly P1Y / monthly P1M × 3 stores), entitlement `pro` (`entl059f4d7f11`), offering `default` (`ofrng9e932e0ae5`, is_current=true) with packages `$rc_lifetime`/`$rc_annual`/`$rc_monthly`. **Lifetime must be `non_consumable`, not `one_time`** — the Test Store rejects `one_time` (allowed: subscription/consumable/non_consumable). Remaining for full production IAP: (1) swap the production iOS key — set `EXPO_PUBLIC_REVENUECAT_IOS_API_KEY` to the `appl_…` key (publishable, not a secret) before the next build; the test key only drives RevenueCat's Test Store. (2) Optionally design a Paywall for offering `default` in the dashboard (otherwise the in-app `FallbackPaywall` is used). The in-flight build `4f187ceb` predates provisioning and uses the embedded test key — a rebuild is needed for the `appl_` key to take effect.
