...
- **INSTALL the new preview build** (`eas build` id `a1eabfce`, 6/15, runtime `a76a90d3`) on the iPhone from its EAS build page — it bakes in the updater + native deps (CSV export, notifications, **expo-audio ka-ching sound**) AND bundles the entry-ordering fix. OTA now works forward from this build. The earlier installed build can't receive the fix OTA because its native fingerprint diverged once the audio module was added.
...
- **Jun 15 — expo-av → expo-audio migration + fresh iOS build**: ka-ching sound now uses `expo-audio ~1.1.1` (expo-av removed); `app.json` plugins updated. Shipped a fresh preview EAS build (`a1eabfce`, runtime `a76a90d3`) that bakes in the audio module + bundles the entry-ordering fix. Root cause the entry fix "still" looked broken on-device: native fingerprint diverged when the audio dep was added, so OTA never reached the old installed build — rebuild required. Build INSTALL_DEPENDENCIES had failed once on `package-firewall.replit.local` URLs injected into the expo lockfile; fix = rewrite them to `registry.npmjs.org` before building. `.agents/memory/eas-expo-build.md`, `.agents/memory/eas-build-lockfile-firewall-url.md`.
